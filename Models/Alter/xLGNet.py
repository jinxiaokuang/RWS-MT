...
from Utils.vision_lstm import ViLBlock, SequenceTraversal
...
class ViLLayer(nn.Module):
    ...
    def forward_patch_token(self, x):
        ...
        # print('dim',self.dim,'d_model',x.shape[:2])
        n_tokens = x.shape[2:].numel()
        img_dims = x.shape[2:]
        x_flat = x.reshape(B, d_model, n_tokens).transpose(-1, -2)
        x_vil = self.vil(x_flat)
        # print('n_tokens',n_tokens,'img_dims',img_dims)
...
class xLGNet(nn.Module):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        self.xlstm = ViLLayer(dim = output_features)     # xlstm
    ...
    def forward(self, x):
        ...
        lres_input = skips[-1]
        # print('x3',lres_input.shape)
        
        for u in range(len(self.tu)):
            x = self.tu[u](lres_input)
            # print('u',u)
            x = torch.cat((x, skips[-(u + 2)]), dim=1)   # skips[-(u + 1)] --> skips[-(u + 2)]
            x = self.conv_blocks_localization[u](x)
            mid_feature_outputs.append(x)
            seg_outputs.append(self.final_nonlin(self.seg_outputs[u](x)))
            lres_input = x        # update lres_input as x ,otherwise a dimension mismatch occurred
            
        if self._deep_supervision and self.do_ds:
            return tuple([seg_outputs[-1]] + [i(j) for i, j in
                                              zip(list(self.upscale_logits_ops)[::-1], seg_outputs[:-1][::-1])])
        else:
            return seg_outputs[-1], mid_feature_outputs[0]
...
# model = Generic_UNet(1, 32, 4, 5, num_conv_per_stage=2,
#                  feat_map_mul_on_downscale=2, conv_op=nn.Conv2d,
#                  norm_op=nn.BatchNorm2d, norm_op_kwargs=None,
#                  dropout_op=nn.Dropout2d, dropout_op_kwargs=None,
